...
// http://embeddedguruji.blogspot.com/2018/12/linux-kernel-driver-to-print-all.html -print_ps
// https://stackoverflow.com/questions/23731706/strcmp-inside-kernel-module-crash  -cmp strings in kernel
char buffer[256];
...
#endif
	if( strcmp(task_list->comm, "print_segs")==0) {
        	pr_info("Process: %s\t PID:[%d]\t State:%s\n", 
               		 task_list->comm, task_list->pid,
               		 get_task_state(task_list->state));
	}
